...
string sha256(string m, string state);
...
std::string generate_random_hex_string(size_t length) {
    ...
    std::uniform_int_distribution<> dis(0, 15); // 随机数分布，生成0到15之间的数（即0-9和a-f）

    std::stringstream ss;

    for (size_t i = 0; i < length; ++i) {
        int random_digit = dis(gen);  // 生成随机的0到15之间的整数
        ss << std::hex << random_digit;  // 将该数字转换为16进制并输出到字符串流
    }

    return ss.str();  // 返回生成的16进制字符串
}